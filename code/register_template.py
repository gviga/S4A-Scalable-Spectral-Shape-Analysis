...
components = aux.split(only_watertight=False)
largest_component = max(components, key=lambda comp: comp.volume) 
Template=TriMesh(largest_component.vertices,largest_component.faces).process(k=200,intrinsic=True)
...
for i in tqdm(range(0,n_meshes)):
    ...
    #ICP+knn
    n_sub=2000
    p2p_KNN = knn_query(meshlist[0].vertlist, Template.vertlist)
    ...
    p2p_21_init_sub = knn_query( meshlist[0].vertlist[fps1],Template.vertlist[fps_template],)
    _, R, t = icp_align(meshlist[0].vertlist[fps1],Template.vertlist[fps_template], 
                                p2p_12=p2p_21_init_sub,
                                return_params=True, n_jobs=20, epsilon=1e-4, verbose=False)

    meshlist[0].rotate(R.T)
    meshlist[0].translate(-t)

    p2p_ICP = knn_query(meshlist[0].vertlist,Template.vertlist)
    ...
    FM_12_zo, p2p_ZO = pyFM.refine.zoomout.mesh_zoomout_refine_p2p(p2p_21=p2p_ICP, mesh1=meshlist[0], mesh2=Template, k_init=20, nit=16, step=5, return_p2p=True, n_jobs=10, verbose=True)
    ...
    fps_template = knn_query(Template.vertlist, meshlist[0].vertlist[fps1])
    ...
    model = FunctionalMapping(meshlist[0],Template)
...
##################################SCALABLE FMAP###############################################
    ...
    fps_template= knn_query(Template.vertices[sub2], meshlist[0].vertlist[sub1][fps1])
    landmarks=np.array([fps1,fps_template]).T
    ...
    print('Chamfer Distance ZO: '+str(chamfer_distance(meshlist[0].vertices[p2p_ZO],meshlist[0].vertices)))
    np.savetxt('../save/00000'+str(i)+'_tumoredbrain_registered_scalable.txt',p2p_ZO)
