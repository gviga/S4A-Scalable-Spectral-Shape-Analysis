...
def compute_sparse_dijkstra_mp(mesh1, subsample, rho, real_rho=None, n_clusters=4, n_jobs=1, correct_dist=True, verbose=True):
    """
    Compute local dijkstra at samples using multiprocessinging and a trick using KMeans to help memory consumption.
    ...
    PROCESSES = min(n_jobs if n_jobs > 0 else multiprocessing.cpu_count(), multiprocessing.cpu_count(), n_clusters)
